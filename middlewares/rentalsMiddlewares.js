...
export async function rentValidate(req, res, next) {
    ...
    try {
        ...
        if (error) return res.status(400).send('Falha ao cadastrar o aluguel');
        else next()
    } catch (e) {
        console.log(e)
        res.sendStatus(500);
...
export async function hasGameAvailable(req, res, next) {
    ...
    try {
        ...
        if (!rentals) next()
        else if (rentals.stockTotal <= rentals.rentTotal) res.status(400).send("Jogo não esta disponível");
        else next()
